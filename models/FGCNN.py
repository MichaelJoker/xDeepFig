...
def FGCNN(
        ...
        name='FGCNN'):    # define FGCNN related parameters
    ...
    with tf.name_scope(name):
        ...
        # Generate a stacked tensor of raw features. All features will be embedded to a same dimension.
        ...
        # Generate a stacked tensor of FGCNN features.
        ...
        fg_inputs = tf.expand_dims(fg_inputs, axis=-1)    # add dimension to the last column

        new_feats_list = list()

        # Feature Generation process: CNN + MLP, generate new features
        ...
        # Combine the new features with the original raw features
        inputs = tf.concat(new_feats_list + [raw_feats], axis=1)
        inputs = split_tensor(inputs, axis=1)

        # Pairwise feature interaction based on inner product
        inputs_fm = InnerProduct(require_logit=False)(inputs)

        # Deep classifier based on traditional Deep Neural Network (DNN)
        ...
        # Utilize Sigmoid activation function to derive the binary label (click or not click) in output layer
